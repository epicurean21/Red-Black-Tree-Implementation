...
public:
    ...
    int addTreatment(int patient_code, string DI, int cost) { // 진료기록 추가하기
        pair<int, Patient *> patient = findPatient(patient_code); // 환자 찾아옴
        if (patient.second == nullptr)
            return -1;
        patient.second->treatRecord.emplace_back(DI, cost); // 가장 최근 진료로 벡터에 추가

        return patient.first;
    }

    void countByPreorder(Node *node, string DI) { // 전위순회로 모든 노드 탐색
        if (node == nullptr)
            return;

        if (node->patient->treatRecord[node->patient->treatRecord.size() - 1].first == DI) // 가장 최근 진료 기록의 병명이 찾던것과 일치한다
            this->diagnose_count++;
        countByPreorder(node->left_child, DI);
        countByPreorder(node->right_child, DI);
    }

    int countDI(string DI) { // 최근 진료 병명 검색을 위한 함수 메인에서 트리의 루트를 참조할 수 없으니 여기서 참조하도록.
        this->diagnose_count = 0;
